...
if(isset($_REQUEST['redeem_point'])){
   ...
   if(isset($_REQUEST['referraled_with']) and $_REQUEST['referraled_with']!='' and !empty($_REQUEST['referraled_with'])){
    ...
    $queryRefer="SELECT `is_rewared` FROM users WHERE id='$id'";
    $resRefer=mysqli_query($conn,$queryRefer);
    if($resRefer) {
    $rewared=mysqli_fetch_array($resRefer);
    
    if($rewared[0]==0||$rewared[0]=="0"){
        	$query="UPDATE users SET points=points+'".$incrment."'  WHERE referral_code='$referraled_with'";
   	$res=mysqli_query($conn,$query);
   	if($res){
   		    $query="UPDATE users SET referraled_with='$referraled_with',is_rewared='1' WHERE id='$id'";
    $res=mysqli_query($conn,$query);
